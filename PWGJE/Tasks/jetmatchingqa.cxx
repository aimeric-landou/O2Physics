...
struct JetMatchingQA {
  ...
  HistogramRegistry registry{
    "registry",
    {
      {"h_jet_hf_pt", "h_jet_hf_pt", {HistType::kTH2F, {{1000, 0.0f, 100.0f, "#it{p}_{T}^{gen} (GeV/#it{c})"}, {1000, 0.0f, 100.0f, "#it{p}_{T}^{det} (GeV/#it{c})"}}}},
      {"h_jet_hf_deta_dphi", "h_jet_hf_deta_dphi", {HistType::kTH2F, {{100, -2. * TMath::Pi(), 2. * TMath::Pi(), "jet #Delta#phi"}, {100, -2.0f, 2.0f, "#Delta#eta"}}}},
      {"h_jet_geo_pt", "geo-matched jets", {HistType::kTH2F, {{1000, 0.0f, 100.0f, "#it{p}_{T}^{gen} (GeV/#it{c})"}, {1000, 0.0f, 100.0f, "#it{p}_{T}^{det} (GeV/#it{c})"}}}},
      {"h_jet_geo_deta_dphi", "geo-matched jets", {HistType::kTH2F, {{100, -2. * TMath::Pi(), 2. * TMath::Pi(), "jet #Delta#phi"}, {100, -2.0f, 2.0f, "#Delta#eta"}}}},
      {"h_jet_pt_pt", "pt-matched jets", {HistType::kTH2F, {{1000, 0.0f, 100.0f, "#it{p}_{T}^{gen} (GeV/#it{c})"}, {1000, 0.0f, 100.0f, "#it{p}_{T}^{det} (GeV/#it{c})"}}}},
      {"h_jet_pt_deta_dphi", "pt-matched jets", {HistType::kTH2F, {{100, -2. * TMath::Pi(), 2. * TMath::Pi(), "jet #Delta#phi"}, {100, -2.0f, 2.0f, "#Delta#eta"}}}},
                  ...
                  BaseJetCollection const& djets, TagJetCollection const& pjets)
  {
    for (const auto& djet : djets) {
      if (djet.has_matchedJetCand() || djet.has_matchedJetGeo()) {
        ...
        registry.fill(HIST("h_jet_det_eta"), djet.tracksIds().size() + 1); // adding HF candidate
      }

      if (djet.has_matchedJetCand()) {
        for (auto& pjet : djet.template matchedJetCand_as<TagJetCollection>()) {
          ...
          registry.fill(HIST("h_jet_hf_pt"), pjet.pt(), djet.pt());
          const auto dphi = -TMath::Pi() + fmod(2 * TMath::Pi() + fmod(djet.phi() - pjet.phi() + TMath::Pi(), 2 * TMath::Pi()), 2 * TMath::Pi());
          registry.fill(HIST("h_jet_hf_deta_dphi"), dphi, djet.eta() - pjet.eta());
      ...
      if (djet.has_matchedJetGeo()) {
        for (auto& pjet : djet.template matchedJetGeo_as<TagJetCollection>()) {
          LOGF(info, "djet %d (pt of %g GeV/c) is geo-matched to %d (pt of %g GeV/c)",
               djet.globalIndex(), djet.pt(), pjet.globalIndex(), pjet.pt());
          registry.fill(HIST("h_jet_geo_pt"), pjet.pt(), djet.pt());
          const auto dphi = -TMath::Pi() + fmod(2 * TMath::Pi() + fmod(djet.phi() - pjet.phi() + TMath::Pi(), 2 * TMath::Pi()), 2 * TMath::Pi());
          registry.fill(HIST("h_jet_geo_deta_dphi"), dphi, djet.eta() - pjet.eta());
      ...
      if (djet.has_matchedJetPt()) {
        for (auto& pjet : djet.template matchedJetPt_as<TagJetCollection>()) {
          LOGF(info, "djet %d (pt of %g GeV/c) is pt-matched to %d (pt of %g GeV/c)",
               djet.globalIndex(), djet.pt(), pjet.globalIndex(), pjet.pt());
          registry.fill(HIST("h_jet_pt_pt"), pjet.pt(), djet.pt());
          const auto dphi = -TMath::Pi() + fmod(2 * TMath::Pi() + fmod(djet.phi() - pjet.phi() + TMath::Pi(), 2 * TMath::Pi()), 2 * TMath::Pi());
          registry.fill(HIST("h_jet_pt_deta_dphi"), dphi, djet.eta() - pjet.eta());
                  ...
                  TagJetCollection const& pjets, BaseJetCollection const& djets)
  {
    for (const auto& pjet : pjets) {
      if (pjet.has_matchedJetCand() || pjet.has_matchedJetGeo()) {
        ...
        registry.fill(HIST("h_jet_gen_eta"), pjet.tracksIds().size() + 1); // adding HF candidate
      }
